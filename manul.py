...
if __name__ == '__main__':
    ...
    start_day = datetime.datetime.strptime('2018-06-16', "%Y-%m-%d").date()

    while True:
        logger.info('{} 开始'.format(start_day))
        logger.info('{} 开始处理原始数据'.format(start_day))

        all_hash_data = origin_data(date=start_day)

        logger.info('{} 原始数据处理完成'.format(start_day))
        logger.info('{} 开始计算数据指标'.format(start_day))
        calculate(all_hash_data)
        logger.info("{} 数据指标计算完成".format(start_day))
